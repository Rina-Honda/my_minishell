...
/*   Updated: 2025/03/18 12:24:30 by rhonda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../libft.h"

static int	ft_putnbr_base(long long n, int base, int specifier, int fd)
{
	char	*str;
	int		byte;
	...
	if (n >= base)
		byte += ft_putnbr_base(n / base, base, specifier, fd);
	if ((specifier == 7) && ((n % base) >= 10))
		write(fd, &str[(n % base) + 6], 1);
	else
		write(fd, &str[n % base], 1);
	byte++;
	return (byte);
}
...
int	ft_dprint_idux(va_list *ap, int specifier, int fd)
{
	...
	len = ft_putnbr_base(n, base, specifier, fd);
	return (len);
}
